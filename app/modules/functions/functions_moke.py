...
def calculate_loop_from_data(data, pulse_voltage, coil_factor=0.92667, window_length=61, polyorder=0):
    ...
    avg_sum = data["Sum"].mean()
    data["Magnetization"] = data["Magnetization"].apply(lambda x: x - offset)
    corr_mag_values = adjust_magnetization(
        data["Magnetization"],
        [(50, 250), (750, 950), (1050, 1250), (1750, 1950)],
        avg_sum,
    )
    # Correct shift between 1st and 2nd pulse
    data["Magnetization"] = corr_mag_values
...
def loop_map_plot(folderpath, database_path):
    ...
    fig.update_layout(
        height=1100,
        width=1100,
        title_text=f"Loop map for {folderpath}",
        showlegend=False,
        plot_bgcolor="white",
    )
